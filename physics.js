(function(){
    ...
    var handleContact = function(b1, b2, depth, xn, yn, zn, restitute){
        ...
        var f1 = mt != 0 ? b1.inv_mass/mt : 0;
        var f2 = mt != 0 ? b2.inv_mass/mt : 0;
    ...
    var Body = Class({
		...
		applyFriction: function(){
			var velocity = this.getVelocity();
			this.ax = velocity[0] != 0 ? 
					(this.ax >= 0 ? this.ax - this.friction : this.ax + this.friction)
					: this.ax;
			this.ay = velocity[1] != 0 ? 
					(this.ay >= 0 ? this.ay - this.friction : this.ay + this.friction)
					: this.ay;
			this.az = velocity[2] != 0 ? 
					(this.az >= 0 ? this.az - this.friction : this.az + this.friction)
					: this.az;
		}
    });
    ...
    vphy = {
        ...
        AABB: Class({
			...
			collideAABB: function(b,restitute)
			{
				//console.log("2 AABB colliding " + restitute);
				
				//TODO: add code to detect AABB collision
				//THREE.Vector3 velocity = this.getVelocity();
				
				
				//console.log("AABB collision: this: ");
				...
				if(restitute == false)
				{
					...
					if(velocity[0] != 0)
					{
						this.setPosition(position[0] - 10,position[1],position[2]);
					}
					else if(velocity[2] != 0)
					{
						this.setPosition(position[0],position[1],position[2] - 10);
					}
					...
					//console.log("AABB collision: b: ");
					//console.log(b.getVelocity());
					b.setVelocity(-b.getVelocity()[0], -b.getVelocity()[1], -b.getVelocity()[2]);
					//console.log(b.getVelocity());
...
/*                var cx = clamp(this.left, this.right, b.x);
                ...
                if(ls < radius*radius){
                    ...
                    var yn = y/l;
                    var zn = z/l;
                    handleContact(b, this, radius-l, xn, yn, zn, restitute);}*/
